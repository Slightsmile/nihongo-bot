// N5 Lessons Page - JavaScript Logic
class N5LessonsPage {
    constructor() {
    ...
    renderLesson(lesson) {
        ...
        const estimatedTime = lesson.estimatedTime || '30 mins';

        // Clean title to remove "Lesson X:" prefix and emoji code
        let cleanTitle = lesson.title
            .replace(/^(👋|📦|📍|⏰|🚃|🍜|🎁|🎨|❤️|🔢|📅|🎌|✍️|🔤|👂)\s*/u, '') // Remove emoji prefix first
            .replace(/^Lesson\s+\-?\d+:\s*/i, '') // Remove "Lesson X:" (handle negative numbers too if any?)
